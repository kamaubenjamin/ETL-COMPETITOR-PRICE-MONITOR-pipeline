...
assert(provider.includes("onAuthStateChange"), "event-driven session restoration is incomplete");
assert(!provider.includes("client.auth.getSession()"), "session restoration must not duplicate Supabase INITIAL_SESSION");
...
assert(provider.includes("resolveSessionProfile(") && provider.includes("createKeyedSingleFlight"), "bounded single-flight session resolution is missing");
assert(provider.includes("signal) => client.get<SafeSessionProfile>"), "timed-out session requests are not cancellable");
