...
Phase 1 implements dependency-free contracts, pagination, read models, safe errors, and structural read ports. Phase 2 adds a deterministic in-memory facade using only those public contracts. Phase 3 adds an API-side adapter using only facade public exports, makes the deterministic facade-backed provider preferred, and preserves the API-local provider for compatibility. Phase 4 verifies dependency direction, payload parity, privacy projections, GET-only routes, request IDs, and security headers, and maps stable facade errors to bounded API outcomes. Live runtime adapters remain pending.
