"""Bounded immutable pagination contracts."""
...
class PageResult(Generic[T]):
    ...
    def __post_init__(self) -> None:
        ...
        try:
            parsed = datetime.fromisoformat(self.snapshot_at.replace("Z", "+00:00"))
        except (AttributeError, ValueError) as exc:
            raise ValueError("snapshot_at must be an ISO-8601 timestamp") from exc
        ...
        if any(not callable(getattr(item, "to_dict", None)) for item in self.items):
            raise ValueError("items must be serializable read models")
