...
def _require_document(document_id: str, request: Request, *, tenant_id: str | None = None) -> dict[str, object]:
    document = get_document_intelligence_provider(request).get_document(document_id, tenant_id=tenant_id)
    if document is None:
...
def list_documents(request: Request, status: str | None = None, document_type: str | None = None, limit: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_PAGE_SIZE), offset: int = Query(0, ge=0)) -> dict[str, object]:
    ...
    rows = get_document_intelligence_provider(request).list_documents(status=status, document_type=document_type, tenant_id=scope.tenant_id)
...
def get_processing(document_id: str, request: Request, limit: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_PAGE_SIZE), offset: int = Query(0, ge=0)) -> dict[str, object]:
    ...
    return paginated_response(get_document_intelligence_provider(request).list_processing(document_id, tenant_id=scope.tenant_id), request_id=request.state.request_id, limit=limit, offset=offset)
