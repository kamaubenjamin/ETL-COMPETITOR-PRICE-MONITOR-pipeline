...
def get_matching(document_id: str, request: Request, limit: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_PAGE_SIZE), offset: int = Query(0, ge=0)) -> dict[str, object]:
    ...
    return paginated_response(get_document_intelligence_provider(request).list_matching(document_id, tenant_id=scope.tenant_id), request_id=request.state.request_id, limit=limit, offset=offset)
