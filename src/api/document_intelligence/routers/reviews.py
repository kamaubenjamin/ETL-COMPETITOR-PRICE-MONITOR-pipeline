...
def _require_review_case(review_case_id: str, *, tenant_id: str | None = None) -> dict[str, object]:
    review_case = local_provider.get_review_case(review_case_id, tenant_id=tenant_id)
...
def list_review_cases(request: Request, status: str | None = None, priority: str | None = None, limit: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_PAGE_SIZE), offset: int = Query(0, ge=0)) -> dict[str, object]:
    ...
    rows = local_provider.list_review_cases(status=status, priority=priority, tenant_id=scope.tenant_id)
...
def get_review_case(review_case_id: str, request: Request) -> dict[str, object]:
    ...
    return success_response(_require_review_case(review_case_id, tenant_id=scope.tenant_id), request_id=request.state.request_id)
...
def get_corrections(review_case_id: str, request: Request, limit: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_PAGE_SIZE), offset: int = Query(0, ge=0)) -> dict[str, object]:
    scope = authorize_read(request, Permission.DOCUMENT_REVIEW, resource_type="review_case", resource_id=review_case_id, conceal_unauthorized_resource=True)
    _require_review_case(review_case_id, tenant_id=scope.tenant_id)
    return paginated_response(local_provider.list_corrections(review_case_id, tenant_id=scope.tenant_id), request_id=request.state.request_id, limit=limit, offset=offset)
...
def list_reprocess_plans(request: Request, limit: int = Query(DEFAULT_PAGE_SIZE, ge=1, le=MAX_PAGE_SIZE), offset: int = Query(0, ge=0)) -> dict[str, object]:
    ...
    return paginated_response(local_provider.list_reprocess_plans(tenant_id=scope.tenant_id), request_id=request.state.request_id, limit=limit, offset=offset)
